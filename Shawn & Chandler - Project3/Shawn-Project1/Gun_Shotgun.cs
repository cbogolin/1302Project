...
namespace ShawnProject1
{
	...
	{
		private static Sound gunshot = new Sound(@"\Application\assets\sound effect\machine gun 2.wav");
		...
		/// Default Shotgun
		/// </summary>
		public ShotGun(GraphicsContext graphics, Vector3 position) 
			: base(new Texture2D(@"\Application\assets\images\guns\shotgun.png", false), graphics, position)
		{
			Graphics = graphics;
			Name = "Shotgun";
			Damage = 2;
			...
			Accuracy = 1f;
			shotsFired = 8;
			BurstDelay = new Timer(45);
			RateOfFire = new Timer(45);
			BulletTexture = new Texture2D(@"\Application\assets\images\bullets\shotgunpellet.png", false);
			CasingTexture = new Texture2D(@"\Application\assets\images\particles\shotgunshell.png", false);
			Status = GunStatus.CanShoot;
			FireType = FireType.PumpAction;
		}
		
		private ShotGun(Texture2D texture, GraphicsContext graphics, Vector3 position, string name, int damage, int amountShot,
		...
		public override GameObject[] Update ()
		{
			...
			case GunStatus.Waiting:
				...
				if (BurstDelay.Status == TimerStatus.JustCompleted)
				{
					...
					if (FireType == FireType.PumpAction)
					{
		...
		public static Gun Create(GraphicsContext graphics, string filePath)
		{
			using (StreamReader reader = new StreamReader(filePath))
			{
				...
				FireType fireType = FireType.PumpAction;
				switch (info[9])
				{
				case "Automatic":
				...
				Texture2D bulletTexture = new Texture2D(info[10], false), casingTexture = new Texture2D(info[11], false), 
					groundTexture = new Texture2D(info[12], false);
				
				return new ShotGun(groundTexture, graphics, Vector3.Zero, info[1], int.Parse(info[2]), int.Parse(info[3]), 
				                        int.Parse(info[4]), int.Parse(info[5]), float.Parse(info[6]), int.Parse(info[7]),
				                        int.Parse(info[8]), fireType, bulletTexture, casingTexture);
			}
			
			return new ShotGun(graphics, Vector3.Zero);
		}
	}
}

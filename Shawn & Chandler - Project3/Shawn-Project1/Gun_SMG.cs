...
namespace ShawnProject1
{
	...
	{	
		...
		/// Default SMG
		/// </summary>
		public SMG(GraphicsContext graphics, Vector3 position) 
			: base(new Texture2D(@"\Application\assets\images\guns\discolaser.png", false), graphics, position)
		{
			player.Loop = true;
			Graphics = graphics;
			Name = "SMG";
			...
			Angle = 0;
			Accuracy = .6f;
			BurstDelay = new Timer(3);
			RateOfFire = new Timer(3);
		...
		private SMG(Texture2D texture, GraphicsContext graphics, Vector3 position, string name, int damage, 
		...
		public static Gun Create(GraphicsContext graphics, string filePath)
		{
			...
			using (StreamReader reader = new StreamReader(filePath))
			{
				...
				return new SMG(groundTexture, graphics, Vector3.Zero, info[1], int.Parse(info[2]), int.Parse(info[3]), 
				                        int.Parse(info[4]), float.Parse(info[5]), int.Parse(info[6]), int.Parse(info[7]),
				                        fireType, bulletTexture, casingTexture);
			}
			
			return new SMG(graphics, Vector3.Zero);
		}
	}
}
